...
public class Bar {
    ...
    private List<Pair> mTickCoordinates = new ArrayList<>();
    ...
    public void drawTicks(Canvas canvas) {
        ...
        // Loop through mTickCoordinates and draw each tick (except final tick).
        for (int i = 0; i < mTickCoordinates.size(); i++) {
            Pair pair = mTickCoordinates.get(i);
            canvas.drawCircle((float)pair.getX(), (Float) pair.getY(), mTickHeight, mTickPaint);
        }

    }

    private void setTickCoordinates(){

        mTickCoordinates.clear();
        // Loop through and add each tick (except final tick).
        for (int i = 0; i < mNumSegments; i++) {
            final float x = i * mTickDistance + mLeftX;
            mTickCoordinates.add(new Pair(x, mY));
        }
        // Add final tick. We add the final tick outside the loop to avoid any
        // rounding discrepancies.
        mTickCoordinates.add(new Pair(mRightX, mY));

    }

    public List<Pair> getTickCoordinates(){
        return mTickCoordinates;
    }
}